...
public class ThreadUtil {
    ...
    private static class CustomThreadFactory implements ThreadFactory {
        ...
        public Thread newThread(Runnable target) {
            Thread t = new Thread(group, target,
                    threadTag + threadNumber.getAndIncrement(), 0);
            if (t.isDaemon()) {
                t.setDaemon(false);
            }
    ...
    private static final int MAXIMUN_POOL_SIZE = CPU_COUNT;
    ...
    /**
     * 懒汉单例模式创建线程池：用于CPU密集型任务
     */
    private static class CpuInternseTargetThreadPoolLazyHolder {
    ...
    public static ThreadPoolExecutor getCpuIntenseTargetThreadPool() {
        return CpuInternseTargetThreadPoolLazyHolder.EXECUTOR;
    }
     ...
     * 混合线程池
     */
    protected static final int MIXED_CORE=0;
